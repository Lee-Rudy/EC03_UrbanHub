...
@router.post("/", response_model=UserResponseDTO, dependencies=[Depends(require_admin)])
...
@router.get("/", response_model=list[UserResponseDTO], dependencies=[Depends(require_admin)])
def get_users(service: UserService = Depends(get_service)):
    return service.get_users()


@router.put("/{user_id}", response_model=UserResponseDTO, dependencies=[Depends(require_admin)])
